...
async fn create_entry(
    ...
    for item in 0..todolist_entries.len() {
        if todolist_entries[item].id > max_id {
            max_id = todolist_entries[item].id;
        }
    }
...
async fn update_entry(
    ...
    let mut todolist_entries = data.todolist_entries.lock().unwrap();
    for intem in 0..todolist_entries.len() {
        if todolist_entries[intem].id == id {
            todolist_entries[intem].title = param_obj.title.clone();
            break;
        }
    }
